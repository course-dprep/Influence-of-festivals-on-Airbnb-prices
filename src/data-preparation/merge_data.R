...
df_calendar_clean <- df_calendar %>% arrange(date) %>% filter(between(date, as.Date("2022-4-30"), as.Date("2022-5-31")))
...
# Keep 
df_merged_clean <- df_merged[,c(1:5,8:10)]

# Save merged data
write_csv(df_merged_clean,file="./gen/data-preparation/temp/df_merged_clean.csv")
